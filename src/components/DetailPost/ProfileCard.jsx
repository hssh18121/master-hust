...
function ProfileCard({imgURL, user}) {
  return (
    <div className="bg-white rounded-md p-8 flex flex-col gap-4 justify-center items-center">
      ...
      <div className="font-semibold text-neutral-800 text-center">{user.name}</div>
      <hr className="h-3 w-40"></hr>
      <div className="flex justify-around text-neutral-400 gap-4">
        {user.github && <LinkIcon link={user.github} icon={<FiGithub />}></LinkIcon>}
        {user.instagram && <LinkIcon link={user.instagram} icon={<FiInstagram />}></LinkIcon>}
        {user.facebook && <LinkIcon link={user.facebook} icon={<FiFacebook />}></LinkIcon>}
      </div>
    </div>
  );
}
